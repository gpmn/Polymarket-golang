...
// GetOrderBookHash returns the order book hash.
...
func (c *ClobClient) GetBuilderTrades(params *BuilderTradeParams, nextCursor string) ([]interface{}, error) {
	...
	for nextCursor != EndCursor {
		url := c.host + GetBuilderTrades + "?builder_code=" + params.BuilderCode + "&next_cursor=" + nextCursor
		if params.ID != "" {
			url += "&id=" + params.ID
		}
		if params.Market != "" {
...
// PostHeartbeat sends a heartbeat to keep orders alive.
